## A simple stimulation for DML estimators that use 5 machine learning methods(default settings) to train the nuisance parameters
# %%import
...
x = np.dot(np.random.randn(n, p), Sigma)
...
Rd2 = Rset[1]
Ry2 = Rset[1]
cd = np.power((np.power(math.pi,2)/3)*Rd2/((1-Rd2)*np.dot(np.dot(np.transpose(theta0),Sigma),theta0)),0.5)
cy = np.power(Ry2/((1-Ry2)*np.dot(np.dot(np.transpose(theta0),Sigma),theta0)),0.5)

d = np.ones(n)
y = np.ones(n)
for i in range(n):
    d[i] = np.exp(np.dot(np.transpose(x[i]),cd*theta0))/(1+np.exp(np.dot(np.transpose(x[i]),cd*theta0)))+v[i]
    y[i] = d[i]+np.dot(np.transpose(x[i]),cd*theta0)+u[i]
# %% DML estimator
def DML(y,d,x,K=2,DML=2,method=LinearRegression,alpha=0.05):
    ...
    for k in range(K):
        ...
        intercept0[k] = np.mean(np.dot(d[main_part],(y[main_part]-g0_hat)))
        theta_hat[k] = np.power(slope[k],-1)*intercept[k]
        # DML estimator using the Neyman orthogonal score function
        slope[k] = np.mean(np.dot(V_hat,d[main_part]))
        intercept[k] = np.mean(np.dot(V_hat,y[main_part]-g0_hat))
